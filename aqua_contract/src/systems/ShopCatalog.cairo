...
pub mod ShopCatalog {
    ...
    impl ShopCatalogImpl of IShopCatalog<ContractState> {
        fn add_new_item(self: @ContractState, price: u256, stock: u256, description: felt252) {
        ...
        fn update_item(self: @ContractState, id: u256, price: u256, stock: u256, description: felt252) {
            let mut world = self.world(@"aqua_stark");
            let mut shop_catalog: ShopCatalogModel = world.read_model(get_contract_address());
            assert(shop_catalog.latest_item_id >= id, 'Item do not exists');
