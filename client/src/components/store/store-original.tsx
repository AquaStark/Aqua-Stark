...
export function StoreOriginal() {
  ...
  const [selectedRarity, setSelectedRarity] = useState<'all' | 'Rare' | 'Legendary' | 'Special'>('all');
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:3001/api/v1/store/items');
        ...
        if (data.success && data.data) {
          ...
          const transformedItems = data.data.map((item: any) => {
            ...
            return {
              ...
              image: imageUrl,
              price: item.price,
              rarity: item.type === 'fish' ? 'Rare' : item.type === 'decoration' ? 'Legendary' : 'Special',
  ...
  const filteredItems = backendItems.filter(item => {
    ...
    if (selectedTab === 'decorations' && item.category !== 'decoration') return false;
