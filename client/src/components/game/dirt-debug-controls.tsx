...
export function DirtDebugControls({
  ...
  return (
    <div
      className={`bg-black/80 backdrop-blur-sm rounded-lg p-4 text-white min-w-64 ${className}`}
    >
      ...
      <div className='space-y-2 mb-4 text-sm'>
        ...
        <div className='flex justify-between'>
          ...
          <span
            className={`font-bold ${cleanlinessScore > 70 ? 'text-green-400' : cleanlinessScore > 40 ? 'text-yellow-400' : 'text-red-400'}`}
          >
            {cleanlinessScore}%
          </span>
        </div>
        ...
        <div className='flex justify-between'>
          <span>Spawner:</span>
          <span
            className={`font-bold ${isSpawnerActive ? 'text-green-400' : 'text-red-400'}`}
          >
