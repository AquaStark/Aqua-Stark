      ...
      <div className="bg-black/80 backdrop-blur-sm rounded-lg border border-blue-400/30 shadow-lg">
        ...
        >
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 rounded-full ${isFeeding ? 'bg-green-400' : 'bg-red-400'}`} />
        ...
        {isExpanded && (
          <div className="p-3 border-t border-blue-400/20 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="text-gray-300">
                  Mode: <span className={isFeeding ? 'text-green-400' : 'text-red-400'}>
                    {isFeeding ? 'Active' : 'Inactive'}
                  </span>
                </div>
                <div className="text-gray-300">
                  Active Foods: <span className="text-blue-400">{activeFoods.length}</span>
                </div>
                <div className="text-gray-300">
                  Consumed: <span className="text-red-400">{consumedFoods.length}</span>
                </div>
                <div className="text-gray-300">
                  Last Update: <span className="text-yellow-400">{formatTime(lastUpdate)}</span>
                </div>
              </div>
            </div>
            ...
            {activeFoods.length > 0 && (
              <div className="space-y-2">
                <div className="text-white text-xs font-semibold">Active Foods</div>
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {activeFoods.map(food => (
                    ...
                    >
                      ...
                      <div className="text-gray-300">
                        Pos: ({food.position.x.toFixed(1)}, {food.position.y.toFixed(1)})
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {consumedFoods.length > 0 && (
              <div className="space-y-2">
                <div className="text-white text-xs font-semibold">Recently Consumed</div>
