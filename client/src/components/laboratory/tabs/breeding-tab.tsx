...
export function BreedingTab({ filteredFish }: BreedingTabProps) {
  ...
  const toggleFilter = (
    filterType: keyof Omit<FilterOptions, 'search' | 'traits'>,
    value: string
  ) => {
    setFilters(prev => {
  ...
  const toggleTraitFilter = (
    traitType: keyof FilterOptions['traits'],
    value: string
  ) => {
    setFilters(prev => {
  ...
  const getFilterSummary = () => {
    const categories = [];
    if (filters.rarity.length > 0)
      categories.push(`${filters.rarity.length} rarities`);
    if (filters.traits.color.length > 0)
      categories.push(`${filters.traits.color.length} colors`);
    if (filters.traits.pattern.length > 0)
      categories.push(`${filters.traits.pattern.length} patterns`);
    if (filters.traits.fins.length > 0)
      categories.push(`${filters.traits.fins.length} fins`);
        ...
        {showFilters && (
          <div className='mt-4 bg-gradient-to-b from-blue-800/60 to-blue-800/40 backdrop-blur-sm rounded-xl border border-blue-700/50 shadow-lg p-3 animate-fade-in animate-duration-300 max-w-full'>
            ...
            <div className='bg-blue-900/20 rounded-lg border border-blue-800/50 p-3 overflow-hidden'>
              ...
              {activeFilterTab === 'color' && (
                <div className='grid grid-cols-2 sm:flex sm:flex-wrap gap-2'>
                  {['Blue', 'Red', 'Green', 'Gold', 'Silver', 'Black'].map(
                    color => (
                      <Button
                        key={color}
                        variant='outline'
              ...
              {activeFilterTab === 'pattern' && (
                <div className='grid grid-cols-2 sm:flex sm:flex-wrap gap-2'>
                  {['Solid', 'Spotted', 'Striped', 'Gradient', 'Marbled'].map(
                    pattern => (
                      <Button
                        key={pattern}
                        variant='outline'
              ...
              {activeFilterTab === 'fins' && (
                <div className='grid grid-cols-2 sm:flex sm:flex-wrap gap-2'>
                  {['Long', 'Short', 'Flowing', 'Spiky', 'Rounded'].map(
                    fins => (
                      <Button
                        key={fins}
                        variant='outline'
