...
function Row({
  ...
  return (
    <div
      className={cn('flex flex-col gap-1', size === 'sm' ? 'gap-0.5' : 'gap-1')}
    >
      {showLabel && (
        <div
          className={cn(
      ...
      <Progress
        value={value}
        indicatorClassName={cn('transition-all', barColor(value))}
        className={cn(size === 'sm' ? 'h-2' : 'h-3')}
      />
    </div>
  );
}
