...
export function DirtCounter({
  ...
  if (variant === 'minimal') {
    return (
      <div
        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/60 backdrop-blur-sm ${className}`}
      >
  ...
  if (variant === 'compact') {
    return (
      <div
        className={`bg-black/80 backdrop-blur-sm rounded-lg px-4 py-3 text-white flex items-center gap-6 border border-gray-700/50 ${className}`}
      >
        {/* Dirt Spots */}
        <div className='flex items-center gap-3'>
          ...
          <div>
            ...
            <div className='flex items-center gap-1'>
              <span
                className={`text-lg font-bold ${getDirtSpotColor(spotCount, maxSpots)}`}
              >
                {spotCount}
              </span>
        ...
        <div className='flex items-center gap-2'>
          <div
            className={`w-2 h-2 rounded-full ${isSpawnerActive ? 'bg-green-400 animate-pulse' : 'bg-gray-500'}`}
          />
  ...
  return (
    <div
      className={`bg-black/85 backdrop-blur-sm rounded-lg p-4 text-white border border-gray-700/50 ${className}`}
    >
      {/* Header */}
      ...
      <div className='grid grid-cols-2 gap-4 mb-4'>
        {/* Current Dirt Spots */}
        <div
          className={`p-3 rounded-lg ${status.bgColor} border border-gray-600/30`}
        >
          ...
          <div className='flex items-baseline gap-1'>
            <span
              className={`text-xl font-bold ${getDirtSpotColor(spotCount, maxSpots)}`}
            >
              {spotCount}
            </span>
          ...
          <div className='w-full bg-gray-700 rounded-full h-1 mt-2'>
            <div
              className={`h-1 rounded-full transition-all duration-300 ${
                spotCount / maxSpots >= 0.8
                  ? 'bg-red-400'
                  : spotCount / maxSpots >= 0.6
                    ? 'bg-orange-400'
                    : spotCount / maxSpots >= 0.4
                      ? 'bg-yellow-400'
                      : 'bg-blue-400'
              }`}
              style={{
                width: `${Math.max(0, Math.min(100, maxSpots > 0 ? (spotCount / maxSpots) * 100 : 0))}%`,
              }}
            />
          </div>
        </div>
        ...
        >
          ...
          <div className='w-full bg-gray-700 rounded-full h-1 mt-2'>
            <div
              ...
              style={{
                width: `${Math.max(0, Math.min(100, cleanlinessScore))}%`,
              }}
            />
          </div>
        </div>
