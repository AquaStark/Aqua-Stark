...
import { EncyclopediaCatalog } from '@/components';
import { EncyclopediaHabitats } from '@/components';
import { EncyclopediaStats } from '@/components';
  ...
  return (
    <>
      <TabsList className='grid grid-cols-3 mb-8 bg-blue-800/40 border border-blue-700/50 backdrop-blur-md rounded-xl overflow-hidden shadow-lg shadow-blue-900/20'>
        ...
        >
          <div
            className='absolute inset-0 bg-gradient-to-r from-cyan-400/20 to-blue-500/20 opacity-0 data-[state=active]:opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none'
            aria-hidden='true'
          />
          <Book
            className='h-5 w-5 mr-2 md:mr-2 transition-transform group-hover:scale-110 duration-300'
            aria-hidden='true'
          />
