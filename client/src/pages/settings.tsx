...
const SettingsItem: React.FC<SettingsItemProps> = ({ icon, title, description, children }) => (
        ...
        <div className="w-full max-w-4xl mx-auto space-y-8">
          <Card className="bg-blue-800/50 backdrop-blur-sm rounded-lg p-6 border border-blue-600/50">
            ...
            <div className="p-0 space-y-4">
              <SettingsItem icon={<Volume2 className="h-6 w-6 text-white" />} title="Sound Effects" description="Enable or disable in-game sound effects.">
                <Switch
                  checked={localSettings.sound_enabled}
                  onCheckedChange={(checked) => handleToggleChange('sound_enabled', checked)}
                  disabled={isLoading}
                />
              </SettingsItem>
            </div>
          ...
          <Card className="bg-blue-800/50 backdrop-blur-sm rounded-lg p-6 border border-blue-600/50">
            ...
            {/* Replaced CardContent */}
            <div className="p-0 space-y-4">
              <SettingsItem icon={<Eye className="h-6 w-6 text-white" />} title="Animations" description="Enable or disable UI animations.">
                <Switch
              ...
              <SettingsItem icon={<Palette className="h-6 w-6 text-white" />} title="UI Theme" description="Choose a visual theme for the game interface.">
                <Select value={localSettings.ui_theme} onValueChange={(value: string) => handleSelectChange('ui_theme', value)} disabled={isLoading}>
          ...
          <Card className="bg-blue-800/50 backdrop-blur-sm rounded-lg p-6 border border-blue-600/50">
            ...
            {/* Replaced CardContent */}
            <div className="p-0 space-y-4">
              <SettingsItem icon={<Bell className="h-6 w-6 text-white" />} title="Notifications" description="Receive notifications for important in-game events.">
                <Switch
          ...
          <Card className="bg-blue-800/50 backdrop-blur-sm rounded-lg p-6 border border-blue-600/50">
            ...
            {/* Replaced CardContent */}
            <div className="p-0 space-y-4">
              <SettingsItem icon={<Globe className="h-6 w-6 text-white" />} title="Language" description="Select your preferred language for the game.">
                <Select value={localSettings.preferred_language} onValueChange={(value: string) => handleSelectChange('preferred_language', value)} disabled={isLoading}>
                  <SelectTrigger className="w-full">
          ...
          <Card className="bg-blue-800/50 backdrop-blur-sm rounded-lg p-6 border border-blue-600/50">
            {/* Replaced CardHeader and CardTitle */}
            <div className="p-0 mb-4 border-b border-blue-600/30 pb-2">
              <h2 className="text-lg font-bold text-white">Gameplay Settings</h2>
            </div>
            {/* Replaced CardContent */}
            <div className="p-0 space-y-4">
              <SettingsItem icon={<Zap className="h-6 w-6 text-white" />} title="Auto Feed" description="Automatically feed your pets when their hunger is low.">
                <Switch
                  checked={localSettings.auto_feed_enabled}
                  onCheckedChange={(checked) => handleToggleChange('auto_feed_enabled', checked)}
                  disabled={isLoading}
                />
              </SettingsItem>
            </div>
