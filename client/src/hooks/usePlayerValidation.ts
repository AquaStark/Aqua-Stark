...
export const usePlayerValidation = () => {
  ...
  const validatePlayer = useCallback(async (walletAddress: string): Promise<PlayerValidationResult> => {
  ...
  const createBackendPlayer = useCallback(async (
    playerId: string, 
  ...
  const syncPlayerToBackend = useCallback(async (
    onChainPlayer: any, 
    walletAddress: string
  ) => {
    try {
