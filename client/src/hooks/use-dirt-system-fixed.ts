...
export function useDirtSystemFixed(config: Partial<DirtSystemConfig> = {}) {
  ...
  const selectRandomDirtType = useCallback((): DirtType => {
    ...
    const types = Object.keys(DIRT_TYPE_CONFIG) as Array<
      keyof typeof DIRT_TYPE_CONFIG
    >;
    const total = types.reduce(
      (sum, t) =>
        sum +
        (weights[t as DirtType] ?? DIRT_TYPE_CONFIG[t].spawnProbability ?? 0.1),
      0
    );
  ...
  const createDirtSpot = useCallback(
    (position: { x: number; y: number }, type?: DirtType): DirtSpot => {
      ...
      const subShapes = Array.from({ length: subShapeCount }, (_, i) => {
        ...
        return {
          size:
            (typeConfig.sizeRange.min +
              Math.random() *
                (typeConfig.sizeRange.max - typeConfig.sizeRange.min)) *
            (0.6 + Math.random() * 0.4),
      ...
      return {
        ...
        size:
          typeConfig.sizeRange.min +
          Math.random() * (typeConfig.sizeRange.max - typeConfig.sizeRange.min),
        opacity:
          typeConfig.opacityRange.min +
          Math.random() *
            (typeConfig.opacityRange.max - typeConfig.opacityRange.min),
        createdAt: Date.now(),
        intensity: 0,
        clickCount: 0,
        subShapes,
        ...
        for (let attempts = 0; attempts < 10; attempts++) {
          ...
          if (isValidPosition(position, prev.spots)) {
            ...
            const newState = {
              ...
              dirtTypeStats: updateDirtTypeStats(
                prev.dirtTypeStats,
                newSpot.type,
                {
  ...
  const removeDirtSpot = useCallback(
    (spotId: number) => {
      setState(prev => {
        ...
        const typeStats = prev.dirtTypeStats[spot.type] || {
          created: 0,
          removed: 0,
          averageTimeToClean: 0,
        };
        ...
        const newAverageTime =
          (typeStats.averageTimeToClean * typeStats.removed + cleaningTime) /
          totalRemoved;
        ...
        dispatchEvent({
          type: 'SPOT_CLEANED',
          payload: { spot, cleaningTime, clickCount },
        });
        dispatchEvent({
          type: 'CLEANLINESS_CHANGED',
          payload: {
  ...
  const removalTimeoutsRef = useRef<Map<number, ReturnType<typeof setTimeout>>>(
    new Map()
  );
  ...
  const clearAllSpots = useCallback(() => {
    setState((prev: DirtSystemState) => {
      ...
      const typeCounts = prev.spots.reduce(
        (acc, s) => {
      ...
      const newState = {
        ...
        efficiency:
          prev.totalSpotsCreated > 0
            ? ((prev.totalSpotsRemoved + clearedCount) /
                prev.totalSpotsCreated) *
              100
            : 0,
  ...
  const updateSpotAging = useCallback(() => {
    ...
    setState(prev => {
      ...
      return {
        ...
        spots: updatedSpots,
        averageSpotAge:
          updatedSpots.length > 0 ? totalAge / updatedSpots.length : 0,
      };
    });
  }, [finalConfig]);
  ...
  const getAnalytics = useCallback((): DirtSystemAnalytics => {
    ...
    const spotsPerMinute =
      sessionDuration > 0 ? state.totalSpotsCreated / sessionDuration : 0;
    let totalMs = 0;
    let removed = 0;
    ...
    return {
      ...
      averageResponseTime,
      dirtTypePreferences: Object.entries(state.dirtTypeStats).reduce(
        (acc, [type, stats]) => {
    ...
    const dirtArea = state.spots.reduce((sum, spot) => {
      ...
      if (spot.subShapes && spot.subShapes.length > 0) {
        ...
        const subShapeArea = spot.subShapes.reduce((subSum, shape) => {
          ...
          return (
            subSum + Math.PI * subRadius * subRadius * (shape.opacity || 1)
          );
        }, 0);
    ...
    if (state.isSpawnerActive) {
      intervalRef.current = setInterval(() => {
        setState(prev => {
          ...
          if (prev.spots.length >= maxSpots || Math.random() > spawnChance)
            return prev;
            ...
            if (isValidPosition(position, prev.spots)) {
              ...
              return {
                ...
                dirtTypeStats: updateDirtTypeStats(
                  ...
                  {
                    created:
                      (prev.dirtTypeStats[newSpot.type]?.created || 0) + 1,
                  }
                ),
              };
            }
